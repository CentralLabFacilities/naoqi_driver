...
void Driver::registerRecorder(const std::string& conv_name, recorder::Recorder& rec, float frequency)
{
  ...
  std::cout << "Not inserting a recorder for: " << conv.name() << std::endl;
...
void Driver::registerConverter(converter::Converter conv, publisher::Publisher pub, recorder::Recorder rec)
{
  ...
  std::cout << "Not registering a recorder (converter) for: " << conv.name() << std::endl;
...
void Driver::registerDefaultSubscriber()
{
  ...
  // ! NOTE using move_base and base_local_planner for navigation
...
void Driver::registerDefaultServices()
{
  ...
  // TODO: Investigate! Update: Maybe it is release image
  registerService(boost::make_shared<service::CameraService>("camera image service", "/naoqi_driver/get_images/investigate", sessionPtr_, AL::kDepthCamera, camera_depth_resolution, camera_depth_fps, AL::kTopCamera, camera_front_resolution, camera_front_fps));
}
